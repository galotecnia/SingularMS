...
jQuery.fn.datetime = function() {
	...
	return this.each(function(){
				...
				var datepicker_def 	= {
							...
							changeYear: true,
							dateFormat: 'yy-mm-dd',
                            // desactivamos el boton hoy (que no funciona xD)
	 ...
	 // Slider
		...
		//Inline editor bind
				...
				function parseTime (obj) {
					...
					if (time.length < 2) {
                        // add six next lines and change date in time array
                        // to fix error in empty input field
                        time = [''+strYear+'-'+iMonth+'-'+iDay,'00:00:00']; 
					}
                    if (time[0].split("-").length <3){
                        time[0] = ''+strYear+'-'+iMonth+'-'+iDay;
                    }
                    if (time[1].split(":").lenght <3){
                        time[1] = '00:00:00';
                    }
